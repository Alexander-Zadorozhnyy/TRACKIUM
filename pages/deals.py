...
class Deals(UserControl):
    def __init__(self, page, user):
        super().__init__()
        self.page = page
        self.user = user

        deals = self.user.db.get_deals(
            self.user.user_id) if self.user.user_id is not None else []
    ...
    def build(self):
        return Column(
            ...
            controls=[
                ...
                Container(
                    height=WINDOW_HEIGHT * (0 if len(self.done) + len(self.active) >= 2
                                            else 0.125 * (2 - len(self.done) + len(self.active)))
                ),
                Row(
                    controls=[
                        Container(
                            content=FilledTonalButton(
                                ...
                                width=WINDOW_WIDTH * 0.6,
                                on_click=lambda _: self.page.go("/add_deal"),
                            ),
